...
def monitor_binance():
    ...
    for symbol in symbols:
        ...
        if oi is not None:
            prev_oi = state['binance'].get(symbol, {}).get('oi')
            if prev_oi is not None and prev_oi > 0:
                change = ((oi - prev_oi) / prev_oi) * 100
                if change > 15:
                    msg = f"[币安] {symbol} 持仓量增长 {change:.2f}%（{prev_oi} -> {oi}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['binance'].setdefault(symbol, {})['oi'] = oi

        # 价格涨幅检测（1 分钟内涨幅超过 5%）
        price = get_binance_price(symbol)
        if price is not None:
            prev_price = state['binance'].get(symbol, {}).get('price')
            if prev_price is not None and prev_price > 0:
                change = ((price - prev_price) / prev_price) * 100
                if change > 5:
                    msg = f"[币安] {symbol} 1 分钟内价格上涨 {change:.2f}%（{prev_price} -> {price}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['binance'].setdefault(symbol, {})['price'] = price
...
def monitor_okx():
    ...
    logging.info(f"OKX监控交易对数：{len(symbols)}")
    for instId in symbols:
        # 资金费率检测
        funding_rate = get_okx_funding_rate(instId)
        if funding_rate is not None and funding_rate < -1:
            msg = f"[OKX] {instId} 资金费率告警：{funding_rate:.2f}% (< -1%)"
            logging.info(msg)
            send_wechat_message(msg)

        # 合约持仓量检测
        oi = get_okx_open_interest(instId)
        if oi is not None:
            prev_oi = state['okx'].get(instId, {}).get('oi')
            if prev_oi is not None and prev_oi > 0:
                change = ((oi - prev_oi) / prev_oi) * 100
                if change > 15:
                    msg = f"[OKX] {instId} 持仓量增长 {change:.2f}%（{prev_oi} -> {oi}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['okx'].setdefault(instId, {})['oi'] = oi

        # 价格涨幅检测（1 分钟内价格上涨超过 5%）
        price = get_okx_price(instId)
        if price is not None:
            prev_price = state['okx'].get(instId, {}).get('price')
            if prev_price is not None and prev_price > 0:
                change = ((price - prev_price) / prev_price) * 100
                if change > 5:
                    msg = f"[OKX] {instId} 1 分钟内价格上涨 {change:.2f}%（{prev_price} -> {price}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['okx'].setdefault(instId, {})['price'] = price
...
def monitor_bybit():
    ...
    logging.info(f"Bybit监控交易对数：{len(symbols)}")
    for symbol in symbols:
        # 资金费率检测
        funding_rate = get_bybit_funding_rate(symbol)
        if funding_rate is not None and funding_rate < -1:
            msg = f"[Bybit] {symbol} 资金费率告警：{funding_rate:.2f}% (< -1%)"
            logging.info(msg)
            send_wechat_message(msg)
        ...
        if oi is not None:
            prev_oi = state['bybit'].get(symbol, {}).get('oi')
            if prev_oi is not None and prev_oi > 0:
                change = ((oi - prev_oi) / prev_oi) * 100
                if change > 15:
                    msg = f"[Bybit] {symbol} 持仓量增长 {change:.2f}%（{prev_oi} -> {oi}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['bybit'].setdefault(symbol, {})['oi'] = oi

        # 价格涨幅检测（1 分钟内价格上涨超过 5%）
        price = get_bybit_price(symbol)
        if price is not None:
            prev_price = state['bybit'].get(symbol, {}).get('price')
            if prev_price is not None and prev_price > 0:
                change = ((price - prev_price) / prev_price) * 100
                if change > 5:
                    msg = f"[Bybit] {symbol} 1 分钟内价格上涨 {change:.2f}%（{prev_price} -> {price}）"
                    logging.info(msg)
                    send_wechat_message(msg)
            state['bybit'].setdefault(symbol, {})['price'] = price
